...
def get_bot_token() -> str:
    """Read the Discord token from the environment, a local file, or interactively."""
    env_token = os.environ.get("DISCORD_TOKEN") or os.environ.get("BOT_TOKEN")
    if env_token:
        return env_token.strip()

    token_file = os.environ.get("BOT_TOKEN_FILE", "bot_token.txt")
    ...
    raise RuntimeError(
        "Bot token not provided. Set DISCORD_TOKEN/BOT_TOKEN or create bot_token.txt."
    )


def get_database_dir() -> str:
    """Return the directory where SQLite files should be stored."""
    db_dir = os.environ.get("DATABASE_DIR") or "db"
    return os.path.abspath(os.path.expanduser(db_dir))


def start_health_server():
    """Start a tiny HTTP health endpoint for Render/UptimeRobot."""
    if os.environ.get("DISABLE_HEALTH_SERVER") == "1":
        return None

    port = os.environ.get("PORT") or os.environ.get("HEALTHCHECK_PORT") or "8080"
    if not os.environ.get("PORT") and not os.environ.get("HEALTHCHECK_PORT") and not os.environ.get("RENDER"):
        return None
...
if __name__ == "__main__":
    ...
    try:
        ...
        startup.error_box("Bot Token Missing", str(e), "Set DISCORD_TOKEN/BOT_TOKEN or create bot_token.txt")
        sys.exit(1)
